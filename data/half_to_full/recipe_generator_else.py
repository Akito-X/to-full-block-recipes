...
recipe_folder = os.path.join(half_to_full, "recipes", "else")
advancement_folder = os.path.join(half_to_full, "advancements", "recipes", "else")
...
for full_block in full_blocks:
    recipe_name = f"{full_block}_block"
    advancement_name = f"{full_block}_block"
    ...
    recipe = {
        ...
        "result": {
            "item": f"minecraft:{full_block}_block",
            "count": 1
        }
    }
    ...
    advancement = {
        ...
        "rewards": {
            "recipes": [
            f"half_to_full:else/{full_block}_block"
            ]
        }
    }
    # 進捗ファイルを保存する
